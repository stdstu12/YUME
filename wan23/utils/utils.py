...
def masks_like(tensor, zero=False, generator=None, p=0.2, latent_frame_zero=8):
            ...
            for u, v in zip(out1, out2):
                ...
                if random_num < p:
                    u[:, :-latent_frame_zero] = torch.normal(
                        ...
                        size=(1,),
                        device=u.device,
                        generator=generator).expand_as(u[:, :-latent_frame_zero]).exp()
                    v[:, :-latent_frame_zero] = torch.zeros_like(v[:, :-latent_frame_zero])
                else:
                    u[:, :-latent_frame_zero] = u[:, :-latent_frame_zero]
                    v[:, :-latent_frame_zero] = v[:, :-latent_frame_zero]
        else:
            for u, v in zip(out1, out2):
