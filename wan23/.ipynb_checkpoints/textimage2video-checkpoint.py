...
class WanTI2V:
    ...
    def i2v(self,
        ...
        """
        # preprocess
        ih, iw = img.shape[2:] #h1, w1 #img.height, img.width
        ...
        z = img#self.vae.encode([img])
        C, F_z, H, W = z.shape
        _, F_target, _, _ = noise.shape
        print(noise.shape,z.shape,"zzzzzzzzzzzzzzzzzzzzz")
        ...
        # sample videos
        latent = noise
        mask1, mask2 = masks_like([noise], zero=True, latent_frame_zero=latent_frame_zero)
        print(mask2[0].shape,z[0].shape,latent.shape,"zzzzzzzzzz")
        #torch.Size([48, 3, 44, 80]) torch.Size([48, 3, 44, 80]) torch.Size([48, 3, 44, 80]) zzzzzzzzzz
        
        #torch.Size([48, 13, 80, 44]) torch.Size([48, 13, 80, 44]) torch.Size([48, 13, 80, 44]) zzzzzzzzzz
        
        # torch.Size([48, 13, 44, 78]) torch.Size([48, 8, 34, 60]) zzzzzzzzzzzzzzzzzzzzz
        # torch.Size([48, 13, 44, 78]) torch.Size([48, 13, 34, 60]) torch.Size([48, 13, 44, 78]) zzzzzzzzzz
    ...
    def t2v_dmd(self,
        ...
        with (
            ...
            for _, t in enumerate(tqdm(timesteps)):
                ...
                noise_pred_cond = teacher_transformer(
                        latent_model_input, t=timestep, **arg_c, flag=False)[0]

#                 noise_pred_uncond = teacher_transformer(
...
#                     noise_pred_cond - noise_pred_uncond)
        ...
        return latents #videos[0] if self.rank == 0 else None
