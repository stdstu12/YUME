...
def rope_apply(x, grid_sizes, freqs, b_, s_, n_, d_, mask_token=None, ids_restore=None, ids_keep=None, rand_num_img=None, flag=None):
    ...
    if not flag: #False:
        ...
        # print(freqs.shape,x.shape,"aeh09q991111111")
        # split freqs
    ...
    def forward(
        ...
        if flag:
            ...
            for u in x:
                ...
                print("seq_len",seq_lens,seq_len,seq_lens1)
                
                seq_len = int(seq_lens[0])
                print("seq_len11",seq_len)
                u = torch.cat([u1,u2],dim=1)
                x_pack.append(u)
                t = t.squeeze()
                print("timestep", t[0], t[-1])
                u1_shape = u1.shape[1]
                u2_shape = u2.shape[1]
                t = torch.cat([
        ...
        if self.mask_ratio is not None and enable_mask:
            ...
            print(t.shape,ids_keep.shape,"wfr32wfsw")
            t_masked = torch.gather(
                t, dim=1, index=ids_keep)
            print(t_masked.shape,"t_maskedt_maskedt_masked")
            t_ori = t
        ...
        if self.mask_ratio is not None and enable_mask:
            ...
            with torch.amp.autocast('cuda', dtype=torch.float32):
                ...
                assert e.dtype == torch.float32 and e0.dtype == torch.float32
            print(e0.shape, e0_ori.shape, t_masked.shape,t_ori.shape, x.shape, x_ori.shape, "1111zzzzzzzzzzzzzz")
            #torch.Size([1, 5169, 6, 3072]) torch.Size([1, 7920, 6, 3072]) torch.Size([1, 5169]) torch.Size([1, 7920]) torch.Size([1, 5169, 3072]) torch.Size([1, 7920, 3072]) 1111zzzzzzzzzzzzzz
        else:
            # time embeddings
            if t.dim() == 1:
        ...
        for block in self.blocks:
            ...
            if cnt_blocks==len_blocks and enable_mask:
                #print(cnt_blocks, x.shape,"1123124")
                ...
                kwargs["seq_lens"] = torch.tensor([x.shape[1]], dtype=torch.long)
                #print(cnt_blocks, x.shape,"1sacfe4tswf4")
                x = block(x, **kwargs)
