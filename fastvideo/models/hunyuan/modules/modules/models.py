...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def init_img_in(self):
        pretrain_weights = {
            'weight': self.img_in.proj.weight.data.clone(),
            'bias': self.img_in.proj.bias.data.clone() if self.img_in.proj.bias is not None else None
        }
        # 扩展权重以适应 self.img_mutil 的输入通道数
        if pretrain_weights['weight'] is not None:
            # 假设 self.img_mutil 的输入通道数是 self.img_in 的两倍
            #print(pretrain_weights['weight'].shape,self.img_mutil.proj.weight.data.shape,"zzzzzzzzzzzzzzzzzzzz")
            #torch.Size([3072, 16, 1, 2, 2]) torch.Size([3072, 32, 1, 2, 2]) zzzzzzzzzzzzzzzzzzzz
            new_weight = torch.cat([pretrain_weights['weight'], pretrain_weights['weight']], dim=1)
            pretrain_weights['weight'] = new_weight
        # 初始化 self.img_mutil 并使用预训练权重
        self.img_mutil = PatchEmbed(self.patch_size, self.in_channels * 2, self.hidden_size, pretrain_weights=pretrain_weights, **self.factory_kwargs)
    ...
    def forward(
        ...
        #print(t.shape,"kkkkkkkkkkkkkkkkkkkkkkkkkkkk")
        vec = self.time_in(t)
