...
def distill_one_step(
    ...
    for _ in range(gradient_accumulation_steps):
        ...
        # Ensure the frame count adheres to the 4n+1 format
        ...
        model_input = latents 
        img = tensor_to_pil(pixel_values_ref_img) # The img has no effect on the result; we only need to obtain the video resolution.

        rand_num_img = random.random()  # i2v or v2v
        if pixel_values_vid.shape[1] <= 33:
            rand_num_img = 0.3
        elif rand_num_img < 0.4:
...
def main(args):
    ...
    # state_dict = load_file("./outputs/checkpoint-1275/diffusion_pytorch_model.safetensors")
    ...
    train_dataset = StableVideoAnimationDataset(height=544, width=960, n_sample_frames=33, sample_rate=1, root_dir=args.root_dir, full_mp4=args.full_mp4)
