...
def create_scaled_videos(folder_path, total_frames=2, H1=256, W1=256):
...
def sample_one(
    ...
    if image_sample:
        ...
        pixel_values_vid = pixel_values_vid[0].unsqueeze(0).repeat(33,1,1,1)
    ...
    for repeat_num in range(repeat_nums):
        ...
        if not wan_i2v.t5_cpu:
            context = wan_i2v.text_encoder(["Generating a city walking video, it is crucial to ensure that the interaction between characters, buildings, vehicles, and the environment follows natural physical laws. The character's movements should have a natural gait and smooth body motions, with steps adapted to the ground conditions, making sure their feet are firmly on the ground and avoiding disconnected movements. The changes in perspective and eye direction should align with their walking trajectory. The proportions and structure of buildings need to be consistent with the character's scale, and the appearance details should reflect real-world materials and lighting changes. Vehicles should move at appropriate speeds and physical responses, following traffic rules, with traffic density adjusted according to the time of day and area. Additionally, changes in weather, lighting, shadows, and dynamic interactions between objects in the urban environment should be accurately simulated, ensuring consistency in both physical and visual elements within the scene. All details must be logical, and sound effects should synchronize with visual effects to enhance immersion. Throughout the entire generation process, strict adherence to the physics engine and natural laws is essential, avoiding perspective errors or unreasonable environmental changes, ensuring that the final video is realistic and smooth."], wan_i2v.device)
        else:
            context = wan_i2v.text_encoder(["Generating a city walking video, it is crucial to ensure that the interaction between characters, buildings, vehicles, and the environment follows natural physical laws. The character's movements should have a natural gait and smooth body motions, with steps adapted to the ground conditions, making sure their feet are firmly on the ground and avoiding disconnected movements. The changes in perspective and eye direction should align with their walking trajectory. The proportions and structure of buildings need to be consistent with the character's scale, and the appearance details should reflect real-world materials and lighting changes. Vehicles should move at appropriate speeds and physical responses, following traffic rules, with traffic density adjusted according to the time of day and area. Additionally, changes in weather, lighting, shadows, and dynamic interactions between objects in the urban environment should be accurately simulated, ensuring consistency in both physical and visual elements within the scene. All details must be logical, and sound effects should synchronize with visual effects to enhance immersion. Throughout the entire generation process, strict adherence to the physics engine and natural laws is essential, avoiding perspective errors or unreasonable environmental changes, ensuring that the final video is realistic and smooth."], torch.device('cpu'))
            context = [context[0].to(wan_i2v.device)]
        context_aam = [context[0]]
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", dtype=torch.bfloat16):
                        for i in range(50):
                            ...
                            import copy
                            arg_c1 = copy.deepcopy(arg_c)
                            arg_c1["context"] = context_aam
                            noise_pred_cond = ema_transformer(
                                    latent_model_input, t=timestep, rand_num_img=0.3, **arg_c1)[0]
                            noise_pred_uncond = ema_transformer(
                                    latent_model_input, t=timestep, rand_num_img=0.3, **arg_null)[0]
                            noise_pred_cond = noise_pred_uncond + 5.0*(noise_pred_cond - noise_pred_uncond)
                            if i+1 == 50:
                                temp_x0 = latent[:,-latent_frame_zero:,:,:] + (0-sampling_sigmas[i])*noise_pred_cond[:,-latent_frame_zero:,:,:]
                            else:
...
def main(args):
    ...
    transformer = wan_i2v.model.to(torch.bfloat16)

    ema_transformer = deepcopy(transformer)
    from safetensors.torch import load_file
    # 加载 safetensors 文件
    state_dict = load_file("/mnt/petrelfs/maoxiaofeng/FastVideo_i2v_pack/outputs_long/checkpoint-300/diffusion_pytorch_model.safetensors")
    # 加载权重
    missing_keys, unexpected_keys = ema_transformer.load_state_dict(state_dict, strict=False)
    ...
    ema_transformer.eval().requires_grad_(False)
