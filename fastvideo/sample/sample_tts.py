            ...
            for step_sample in range(sample_num):
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", dtype=torch.bfloat16):
                        # Time Travel Configuration Parameters
                        time_travel_step = 2 # Time travel step length (l)
                        time_travel_interval = 2 # Time travel interval in steps (executes every 10 steps)
                        time_travel_repeat = 1 # Repeat count (r)
                        ...
                        for i in range(sample_step):
                            ...
                            if time_travel_interval > 0 and i % time_travel_interval == 0:
                                ...
                                if time_travel_interval > 0:
                                    

                                    # # 合成新噪声
                                    noise_travel = noise_ori_pred_cond
                                    ...
                                    for j in range(i+1, travel_step):
                                        ...
                                        if sde: 
                                            ...
                                            eta = 0.3
                                            delta_t = (sampling_sigmas[j] - sampling_sigmas[j+1])
                                            if delta_t < 0:
                                                delta_t = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sde", action="store_true")  # TODO
